...
export function makeBasicLandCard(name) {
  ...
  const producedMana = BASIC_LAND_MANA[name];
  return {
    name,
    cmc: 0,
    mana_cost: "",
    ...
    produced_mana: producedMana ? [producedMana] : [],
...
function hasDirectManaProduction(text) {
  const directAmount = "(?:one|two|three|four|five|six|x|\\d+) mana|(?:\\{[wubrgc]\\})+";
  return new RegExp(`(?:^|\\n)add (?:${directAmount})`).test(text)
    || new RegExp(`(?:\\{t\\}|sacrifice [^:]+):\\s*add (?:${directAmount})`).test(text);
}
...
export function getRoles(card) {
  ...
  const detected = {
    ramp: !type.includes("land") && (hasAny(text, ROLE_PATTERNS.ramp) || hasDirectManaProduction(text)),
  ...
  const overrides = card?.role_overrides && typeof card.role_overrides === "object" ? card.role_overrides : {};
  for (const [role, active] of Object.entries(overrides)) {
    if (role in detected && typeof active === "boolean") detected[role] = active;
  }
  return detected;
}
...
function evidenceForRole(role, card, name) {
  ...
  if (card.role_overrides && typeof card.role_overrides[role] === "boolean") {
    return {
      role,
      cardName: name || card.name,
      reason: `${ROLE_LABELS[role] || role} was ${card.role_overrides[role] ? "added" : "removed"} for this session.`,
      confidence: "high",
      matchingRule: "session-role-override",
  ...
  if (role === "ramp" && !type.includes("land") && hasDirectManaProduction(text)) {
    return {
      role,
      cardName: name || card.name,
      reason: "Card has direct mana-production text.",
      confidence: "medium",
      matchingRule: "manual heuristic:direct-mana-production",
...
export function applyRoleOverridesToCardMap(cardMap = {}, roleOverrides = {}) {
  const clones = new Map();
  return Object.fromEntries(Object.entries(cardMap).map(([key, card]) => {
    if (!card || typeof card !== "object") return [key, card];
    const overrides = roleOverrides[normalizeName(card.name || key)];
    if (!overrides || typeof overrides !== "object" || !Object.keys(overrides).length) return [key, card];
    if (!clones.has(card)) clones.set(card, { ...card, role_overrides: { ...overrides } });
    return [key, clones.get(card)];
  }));
}
