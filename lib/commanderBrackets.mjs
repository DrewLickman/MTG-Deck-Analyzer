...
export const GAME_CHANGERS_VERSION = "local-2026-06-starter";
...
const GAME_CHANGER_SET = new Set(GAME_CHANGERS.map(normalizeName));
...
export function isGameChangerName(name) {
  return GAME_CHANGER_SET.has(normalizeName(name));
}

function allDeckEntries(deck) {
  return [
    ...deck.commanders,
    ...deck.companions,
    ...deck.main,
...
function namesInDeck(deck) {
  return new Set(allDeckEntries(deck).map((entry) => normalizeName(entry.name)));
}
