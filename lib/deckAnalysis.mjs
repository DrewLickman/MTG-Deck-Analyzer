...
function analyzeCandidateCards(entries, cardMap, identityCards, stats, scores, options) {
  ...
  return entries.map((entry) => {
    ...
    if ((roles.ramp && stats.rampCount < rampFloor) || (roles.removal && stats.removalCount < settings.removalTarget) || (roles.boardWipe && stats.boardWipeCount < settings.wipesTarget) || scored.score >= 4) {
      recommendation = "add";
...
function buildCutCandidates({ scores, landEntries, cardMap, identityCards, stats, settings, structure, scorecard, deckSizePlan, coreCards = [] }) {
  ...
  const nonLandCandidates = [...scores]
    ...
    .map((score) => {
      ...
      const confidence = evaluation.rank >= 6 && !evaluation.keepRisk.length
        ? "high"
        : evaluation.rank >= 3
          ? "medium"
          : "low";
...
function buildUpgrades(stats, cutCandidates, sideboardAnalysis, consideringAnalysis, settings, manaFit) {
  const cuts = [...cutCandidates].filter((candidate) => candidate.confidence !== "low").slice(0, 5);
  const adds = [...sideboardAnalysis, ...consideringAnalysis]
    .filter((item) => item.recommendation !== "skip")
...
function buildNextSteps({ deckSizePlan, manaFit, interactionProfile, cardFlowProfile, resilienceProfile, winPlan, upgrades, cutCandidates, answerGaps }) {
  ...
  if (manaFit?.status !== "good") {
    ...
    const title = rampLow
      ? `Add ${countRange(manaFit.rampAddRange.min, manaFit.rampAddRange.max, "ramp piece")}`
      : landLow
        ? `Add ${countRange(manaFit.landAddRange.min, manaFit.landAddRange.max, "land")}`
        : "Tune mana support";
    add({
      id: "mana-support",
      category: "mana",
