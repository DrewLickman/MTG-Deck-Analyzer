...
function buildManaFit(nonLandEntries, cardMap, commanderCards, stats, settings) {
  ...
  const recommendation = landLow && rampLow
    ? "Mana support looks light for this curve; add lands, cheap ramp, or lower the top end before adding more expensive cards."
    : landLow
      ? "Mana support looks light for this curve; add lands, MDFCs, or reduce expensive spells."
      : rampLow
        ? "Mana support looks light for this curve; add cheap acceleration that helps cast commander/core cards on time."
        : landHigh || rampHigh
          ? "Mana support looks heavy for this curve; trim lands or redundant ramp only if card flow and early plays stay stable."
          : "Mana support fits the current curve reasonably well.";
...
function buildUpgrades(stats, cutCandidates, sideboardAnalysis, consideringAnalysis, settings, manaFit) {
  ...
  if (adds.length) {
    return adds.slice(0, 5).map((add, index) => {
      ...
      return {
        ...
        cutScore: cut.score,
        add: add.name,
        reason: `${add.reason} Cut candidate reason: ${cut.reasons?.[0] || "lowest available cut candidate."}`,
        expensive: false,
      };
    });
  }
  ...
  return cuts.slice(0, 4).map((cut) => ({
    ...
    cutScore: cut.score,
    add: suggestedAdd,
    reason: `${cut.reasons?.[0] || "Lower-impact slot."} Use this slot to shore up the clearest measured gap.`,
    expensive: false,
  }));
}
...
function buildRoadmap({ priorityFindings, scorecard, cutCandidates, upgrades, structure, manaFit, deckSizePlan, coreCards }) {
  const activeFindings = (priorityFindings || []).filter((finding) => finding.severity !== "notice").slice(0, 3);
  const lowScores = (scorecard || []).filter((item) => !item.ignored && item.score < 70).sort((a, b) => a.score - b.score).slice(0, 3);
  const addPriorities = [];
  ...
  for (const item of lowScores) {
    if (addPriorities.some((priority) => priority.label === item.label)) continue;
    addPriorities.push({
      label: item.label,
      action: item.adjustments?.[0] || item.summary,
      reason: item.summary,
    });
  }

  if (manaFit?.status !== "good" && !addPriorities.some((priority) => priority.label === "Mana Fit")) {
    addPriorities.unshift({
      label: "Mana Fit",
      action: manaFit.recommendation,
      reason: `${manaFit.currentLands} lands and ${manaFit.currentRamp} ramp pieces for this curve.`,
    });
  }
  ...
  return {
    status: activeFindings.length ? "needs-work" : "stable",
    headline: activeFindings[0]?.action || "Tune around the strongest game-plan pieces and playtest results.",
    steps: addPriorities.slice(0, 5),
    cutPriorities,
    upgradePairs,
...
function buildActionPlan({ priorityFindings, cutCandidates, upgrades, roadmap, deckSizePlan, manaFit }) {
  const tasks = [];
  const activeFindings = (priorityFindings || []).filter((finding) => finding.severity !== "notice");
  ...
  for (const finding of activeFindings) {
    const priority = finding.severity === "critical" ? "required" : "recommended";
    tasks.push({
      id: `finding-${normalizeName(finding.label)}`,
      type: finding.label === "Deck has too many cards" ? "cut" : "fix",
      priority,
      label: finding.label,
      detail: finding.detail,
      action: finding.action,
      tab: finding.label === "Deck has too many cards" ? "cuts" : finding.label.toLowerCase().includes("mana") ? "mana" : "scorecard",
      relatedCards: finding.label === "Deck has too many cards" ? topCuts.map((candidate) => candidate.name) : [],
    });
  }

  if (deckSizePlan?.cutsNeeded > 0 && !tasks.some((task) => task.type === "cut")) {
    tasks.unshift({
      id: "required-size-cuts",
      type: "cut",
      priority: "required",
      label: `Cut ${deckSizePlan.cutsNeeded} card${deckSizePlan.cutsNeeded === 1 ? "" : "s"}`,
      detail: deckSizePlan.message,
      action: `Start with ${topCuts.map((candidate) => candidate.name).join(", ") || "the highest-ranked cut candidates"}.`,
      tab: "cuts",
      relatedCards: topCuts.map((candidate) => candidate.name),
    });
  }

  if (manaFit?.status !== "good" && !tasks.some((task) => task.id === "finding-mana-support-is-light" || task.id === "finding-mana-support-needs-review")) {
    tasks.push({
      id: "mana-fit",
      type: "fix",
      priority: manaFit.status === "bad" ? "required" : "recommended",
      label: "Tune mana support",
      detail: `${manaFit.currentLands} lands and ${manaFit.currentRamp} ramp pieces against a ${manaFit.landRange.min}-${manaFit.landRange.max} land and ${manaFit.rampRange.min}-${manaFit.rampRange.max} ramp curve-fit range.`,
      action: manaFit.recommendation,
      tab: "mana",
      relatedCards: [],
    });
  ...
  if (topAdds.length) {
    tasks.push({
      id: "upgrade-adds",
      type: "add",
      priority: activeFindings.length ? "recommended" : "optional",
      label: "Test the best upgrade swaps",
      detail: topAdds.map((upgrade) => `${upgrade.add} over ${upgrade.cut}`).join("; "),
      action: "Use these as the first add/cut experiments before browsing generic staples.",
      tab: "upgrades",
      relatedCards: topAdds.flatMap((upgrade) => [upgrade.add, upgrade.cut]).filter(Boolean),
    });
  }
...
export function buildLocalAnalysis(deck, cardMap, options = {}) {
  ...
  const roadmap = buildRoadmap({ priorityFindings, scorecard, cutCandidates, upgrades, structure, manaFit, deckSizePlan, coreCards });
  const actionPlan = buildActionPlan({ priorityFindings, cutCandidates, upgrades, roadmap, deckSizePlan, manaFit });
