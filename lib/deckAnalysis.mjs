...
function buildGenericManaCost(entries, cardMap) {
  return entries.reduce((sum, entry) => {
    const card = findCard(cardMap, entry.name);
    const generic = Array.from(String(getManaCost(card) || "").matchAll(/\{(\d+)\}/g))
      .reduce((cardSum, match) => cardSum + Number(match[1]), 0);
    return sum + generic * entry.qty;
  }, 0);
}
...
function scoreEntry(entry, cardMap, identityCards, stats, options = {}) {
  const { coreCards = [], commanderNames = [], settings = DEFAULT_ANALYSIS_SETTINGS, manaFit = null } = options;
  const card = findCard(cardMap, entry.name);
  ...
  if (roles.ramp) {
    score += stats.rampCount < (manaFit?.rampRange?.min ?? settings.rampTarget) ? 3 : 2;
    reasons.push("ramp");
  }
  ...
  return {
    ...
    roleEvidence: getRoleEvidence(card, entry.name),
...
function buildSynergyClusters(nonLandEntries, cardMap, identityCards = [], coreCards = []) {
  const clusters = [];
  const primarySignalOrder = ["tokens", "artifacts", "graveyard", "counters", "sacrifice", "combat", "spells"];
  const signalRank = new Map(primarySignalOrder.map((key, index) => [key, index + 1]));
  const coreSet = new Set((coreCards || []).map(normalizeName));
  const commanderIdentityCards = identityCards.filter((card) => !coreSet.has(normalizeName(card?.name)));
  const identitySignals = SIGNALS
    .filter((signal) => commanderIdentityCards.some((card) => signal.test(card)))
  ...
  if (coreCards.length || identitySignals.length) {
    const supportCards = nonLandEntries
      .filter((entry) => {
        const card = findCard(cardMap, entry.name);
        return coreSet.has(normalizeName(entry.name)) || getSharedSignals(card, commanderIdentityCards).length;
      })
  ...
  for (const signal of SIGNALS) {
    ...
    if (cards.length >= 2) {
      ...
      clusters.push({
        ...
        cards,
        desc: signal.desc,
        identity: identityOverlap,
        primaryRank: signalRank.get(signal.key) ?? primarySignalOrder.length + 1,
      });
    }
  }
  ...
  if (!clusters.length) {
    ...
    if (roleCards.length) {
      clusters.push({
        ...
        primaryRank: primarySignalOrder.length + 2,
      });
    }
  }
  const assignedCards = new Set();
  return clusters
    // Prefer the most specific theme before broad spells-matter overlap, so a
    // token maker primarily reads as token support and only tags spells secondarily.
    .sort((a, b) => (a.primaryRank ?? 99) - (b.primaryRank ?? 99) || b.cards.length - a.cards.length)
    .slice(0, 5)
    .map((cluster) => {
      const primaryCards = [];
      const secondaryCards = [];
      for (const name of [...new Set(cluster.cards)]) {
        const key = normalizeName(name);
        if (assignedCards.has(key)) secondaryCards.push(name);
        else {
          assignedCards.add(key);
          primaryCards.push(name);
        }
      }
      return {
        ...cluster,
        cards: primaryCards,
        secondaryCards: secondaryCards.slice(0, 6),
      };
    })
    .filter((cluster) => cluster.cards.length);
}

function buildConsistencyFlags(stats, deck, settings, manaFit) {
  ...
  return [
    ...
    { ok: manaFit.status !== "bad", text: `${manaFit.currentLands} lands and ${manaFit.currentRamp} ramp pieces against a curve-fit range of ${manaFit.landRange.min}-${manaFit.landRange.max} lands and ${manaFit.rampRange.min}-${manaFit.rampRange.max} ramp.` },
...
function buildWeaknesses(stats, settings, manaFit) {
  const weaknesses = [];
  ...
  if (manaFit.status === "bad") weaknesses.push({ severity: "critical", label: "Mana support is light", desc: manaFit.recommendation });
  else if (manaFit.status === "warn") weaknesses.push({ severity: "warning", label: "Mana support needs review", desc: manaFit.recommendation });
  ...
  if (!weaknesses.length) weaknesses.push({ severity: "minor", label: "No major structural gap", desc: "Core mana fit, curve, wipe, and interaction checks are coherent." });
  return weaknesses;
}
...
function buildRoleBalance(nonLandEntries, cardMap, settings, manaFit) {
  ...
  const specs = [
    { key: "ramp", label: "Mana Development", target: `${manaFit.rampRange.min}-${manaFit.rampRange.max}`, warnAt: Math.max(0, manaFit.rampRange.min - 2), goodAt: manaFit.rampRange.min, detail: "Gets the deck to its commander and midgame engines on time for this curve." },
  ...
  return ["0-1", "2", "3", "4", "5+"].map((key) => {
    ...
    return {
      ...
      detail: commanderNames.length
        ? `${commanderNames.join(" and ")} ${commanderNames.length === 1 ? "costs" : "cost"} this much.`
...
function countRange(minimum, maximum, singular, plural = `${singular}s`) {
  if (maximum <= minimum) return `${minimum} ${minimum === 1 ? singular : plural}`;
  return `${minimum}–${maximum} ${plural}`;
}

function buildManaFit(nonLandEntries, cardMap, commanderCards, stats, settings) {
  let earlyCount = 0;
  let topEndCount = 0;
  let cheapRampCount = 0;
  const nonLandQty = nonLandEntries.reduce((sum, entry) => sum + entry.qty, 0) || 1;

  for (const entry of nonLandEntries) {
    const card = findCard(cardMap, entry.name);
    const cmc = card?.cmc ?? 0;
    const roles = getRoles(card);
    if (cmc <= 2) earlyCount += entry.qty;
    if (cmc >= 5) topEndCount += entry.qty;
    if (roles.ramp && cmc <= 2) cheapRampCount += entry.qty;
  }

  const commanderMaxCmc = Math.max(0, ...commanderCards.map((card) => card?.cmc ?? 0));
  const earlyShare = earlyCount / nonLandQty;
  const topEndShare = topEndCount / nonLandQty;
  const reasons = [];
  let curvePressure = 0;

  if (stats.avgCmc <= 2.45 && topEndShare <= 0.14) {
    curvePressure -= 2;
    reasons.push("Low average mana value and light top end reduce the need for extra mana support.");
  } else if (stats.avgCmc <= 2.8 && topEndShare <= 0.18) {
    curvePressure -= 1;
    reasons.push("The curve is relatively lean, so the deck can operate with a lighter mana package.");
  }
  if (stats.avgCmc > 3.35) {
    curvePressure += 1;
    reasons.push(`Average mana value ${stats.avgCmc} asks for more mana support than a low-curve deck.`);
  }
  if (stats.avgCmc > 3.75) {
    curvePressure += 1;
    reasons.push("The average mana value is high enough that missed land drops or slow ramp become more punishing.");
  }
  if (topEndShare >= 0.22) {
    curvePressure += 1;
    reasons.push(`${topEndCount} cards cost five or more, increasing top-end pressure.`);
  }
  if (topEndShare >= 0.32) {
    curvePressure += 1;
    reasons.push("A large share of the deck sits at five-plus mana.");
  }
  if (commanderMaxCmc >= 5) {
    curvePressure += 1;
    reasons.push(`Commander mana value ${commanderMaxCmc} raises the cost of getting the deck online.`);
  }
  if (earlyShare < 0.22 && stats.avgCmc > 3.1) {
    curvePressure += 1;
    reasons.push("Few cheap nonland cards make early turns more dependent on lands and ramp.");
  }
  if (cheapRampCount >= 5) {
    curvePressure -= 1;
    reasons.push(`${cheapRampCount} cheap ramp pieces help absorb curve pressure.`);
  }

  const landOffset = curvePressure >= 4 ? 2 : curvePressure >= 2 ? 1 : curvePressure <= -2 ? -2 : curvePressure <= -1 ? -1 : 0;
  const rampOffset = curvePressure >= 4 ? 3 : curvePressure >= 2 ? 2 : curvePressure >= 1 ? 1 : curvePressure <= -2 ? -3 : curvePressure <= -1 ? -2 : 0;
  const landRange = {
    min: clampNumber(settings.landsMin + landOffset, 32, 42),
    max: clampNumber(settings.landsMax + landOffset, 34, 44),
  };
  const rampRange = {
    min: clampNumber(settings.rampTarget - 2 + rampOffset, 3, 16),
    max: clampNumber(settings.rampTarget + 1 + rampOffset, 5, 18),
  };

  const landLow = stats.landCount < landRange.min;
  const rampLow = stats.rampCount < rampRange.min;
  const landHigh = stats.landCount > landRange.max + 2;
  const rampHigh = stats.rampCount > rampRange.max + 4;
  const severeLow = stats.landCount < landRange.min - 2 || stats.rampCount < rampRange.min - 2;
  const status = severeLow ? "bad" : landLow || rampLow || landHigh || rampHigh ? "warn" : "good";
  const landDeficit = Math.max(0, landRange.min - stats.landCount);
  const rampDeficit = Math.max(0, rampRange.min - stats.rampCount);
  const landAddRange = {
    min: landDeficit,
    max: Math.max(landDeficit, landRange.max - stats.landCount),
  };
  const rampAddRange = {
    min: rampDeficit,
    max: Math.max(rampDeficit, rampRange.max - stats.rampCount),
  };
  const recommendation = landLow && rampLow
    ? `Add ${countRange(landAddRange.min, landAddRange.max, "land or MDFC", "lands or MDFCs")} and ${countRange(rampAddRange.min, rampAddRange.max, "ramp piece")}.`
    : landLow
      ? `Add ${countRange(landAddRange.min, landAddRange.max, "land or MDFC", "lands or MDFCs")}.`
      : rampLow
        ? `Add ${countRange(rampAddRange.min, rampAddRange.max, "ramp piece")}.`
        : landHigh || rampHigh
          ? "Mana support is above this curve's target; trim only redundant lands or ramp."
          : "Mana support fits the current curve.";

  return {
    status,
    landRange,
    rampRange,
    currentLands: stats.landCount,
    currentRamp: stats.rampCount,
    landDeficit,
    rampDeficit,
    landAddRange,
    rampAddRange,
    ...
    cheapRampCount,
    commanderMaxCmc,
    averageManaValue: stats.avgCmc,
    reasons: [
      ...reasons,
      `Suggested range: ${landRange.min}-${landRange.max} lands and ${rampRange.min}-${rampRange.max} ramp for this curve.`,
    ],
    recommendation,
  };
}
...
function hasBlueIdentity(commanderCards = []) {
  return commanderCards.some((card) => {
    const identity = card?.color_identity || [];
    return identity.includes("U") || getManaColorKeys(card).includes("U");
  });
}

function buildAnswerGaps(stats, interactionProfile, resilienceProfile, commanderCards = []) {
  const blueIdentity = hasBlueIdentity(commanderCards);
  const specs = [
    ...
    ...(blueIdentity ? [{ key: "counterspells", label: "Counterspells / stack interaction", count: interactionProfile.stackInteraction, expected: "1–4", ok: interactionProfile.stackInteraction >= 1 }] : []),
...
function buildWinPlan(nonLandEntries, cardMap, synergyClusters) {
  ...
  const setupCandidates = [
    ...roleExamples(nonLandEntries, cardMap, "ramp", 6),
    ...roleExamples(nonLandEntries, cardMap, "cardSelection", 6),
    ...roleExamples(nonLandEntries, cardMap, "draw", 6),
  ];
  const assignedCards = new Set();
  const stageCards = (cards) => cards.filter((name) => {
    const key = normalizeName(name);
    if (assignedCards.has(key)) return false;
    assignedCards.add(key);
    return true;
  }).slice(0, 6);
  const setup = stageCards(setupCandidates);
  const engine = stageCards([...engines, ...payoffs]);
  const finish = stageCards([...finishers, ...payoffs]);
  ...
  return {
    ...
    payoffs,
    stages: [
      { key: "setup", label: "Setup", cards: setup, summary: "Develop mana and smooth the first turns." },
      { key: "engine", label: "Engine", cards: engine, summary: "Deploy the repeatable pieces that advance the plan." },
      { key: "finish", label: "Finish", cards: finish, summary: "Convert the established board into a win." },
    ],
    status,
    note: status === "good"
...
function buildDeckSizePlan(deck) {
  const commanderCount = deck.commanders.reduce((sum, entry) => sum + entry.qty, 0);
  const totalCards = deck.cardCount + commanderCount;
  const allowedTotal = commanderCount > 1 ? 101 : 100;
  const targetTotal = 100;
  const cutsNeeded = totalCards > allowedTotal ? totalCards - targetTotal : 0;
  return {
    totalCards,
    commanderCount,
    allowedTotal,
    targetTotal,
    cutsNeeded,
    message: cutsNeeded > 0
      ? `Deck has ${totalCards} total cards. Cut ${cutsNeeded} card${cutsNeeded === 1 ? "" : "s"} to reach ${targetTotal}.`
      : `Deck has ${totalCards} total cards and is within the ${allowedTotal}-card threshold.`,
  };
}

function buildPriorityFindings(stats, interactionProfile, resilienceProfile, cardFlowProfile, winPlan, bracket, scorecard = [], answerGaps = [], manaFit, deckSizePlan) {
  const findings = [];

  if (deckSizePlan?.cutsNeeded > 0) {
    findings.push({
      severity: "critical",
      label: "Deck has too many cards",
      detail: `${deckSizePlan.totalCards} total cards detected. This deck is allowed ${deckSizePlan.allowedTotal} before size cuts are required; target total is ${deckSizePlan.targetTotal}.`,
      action: `Cut ${deckSizePlan.cutsNeeded} card${deckSizePlan.cutsNeeded === 1 ? "" : "s"}, starting with the highest-confidence cut candidates.`,
    });
  }
  ...
  if (manaFit?.status === "bad" || manaFit?.status === "warn") {
    findings.push({
      severity: manaFit.status === "bad" ? "critical" : "warning",
      label: manaFit.status === "bad" ? "Mana support is light" : "Mana support needs review",
      detail: `${manaFit.currentLands} lands and ${manaFit.currentRamp} ramp pieces. Curve-fit range is ${manaFit.landRange.min}-${manaFit.landRange.max} lands and ${manaFit.rampRange.min}-${manaFit.rampRange.max} ramp.`,
      action: manaFit.recommendation,
    });
  }
...
function analyzeCandidateCards(entries, cardMap, identityCards, stats, scores, options) {
  ...
  return entries.map((entry) => {
    ...
    const rampFloor = options.manaFit?.rampRange?.min ?? settings.rampTarget;
    const landFloor = options.manaFit?.landRange?.min ?? settings.landsMin;
    if ((roles.ramp && stats.rampCount < rampFloor) || (land && stats.landCount < landFloor) || (roles.removal && stats.removalCount < settings.removalTarget) || (roles.boardWipe && stats.boardWipeCount < settings.wipesTarget) || scored.score >= 4) {
      recommendation = "add";
    } else if (scored.score <= 0) {
      recommendation = "skip";
    }
    const benefit = roles.ramp && stats.rampCount < rampFloor
      ? `Adds ramp toward the ${options.manaFit.rampRange.min}–${options.manaFit.rampRange.max} target.`
      : land && stats.landCount < landFloor
        ? `Adds a land toward the ${options.manaFit.landRange.min}–${options.manaFit.landRange.max} target.`
        : roles.removal && stats.removalCount < settings.removalTarget
        ? `Adds targeted interaction toward the ${settings.removalTarget}-card target.`
        : roles.boardWipe && stats.boardWipeCount < settings.wipesTarget
          ? `Adds board control toward the ${settings.wipesTarget}-card target.`
          : scored.note
            ? `Supports the deck through ${scored.note}.`
            : "Adds a plausible role-player for playtesting.";
    const cutText = lowest ? ` Test against ${lowest.name}.` : "";
    return {
      name: entry.name,
      roles: roleKeys,
      land,
      ...
      reason: recommendation === "add"
        ? `${benefit}${cutText}`
        : recommendation === "skip"
          ? "Does not clearly address a current deck need."
          : `${benefit}${cutText}`,
    };
  });
}

const ROLE_REPLACEMENT_NEEDS = {
  ramp: "cheap ramp",
  draw: "repeatable draw or card selection",
  removal: "flexible targeted removal",
  boardWipe: "board control",
  protection: "protection for commander/core engines",
  recursion: "recursion or rebuild tools",
  finisher: "a clearer finisher",
  payoff: "a payoff tied to the deck identity",
  engine: "a repeatable engine tied to the deck identity",
};

const CUT_ROLE_KEYS = ["ramp", "draw", "cardSelection", "removal", "boardWipe", "protection", "recursion", "engine", "payoff", "finisher"];

function roleIsThin(role, stats, settings, structure) {
  if (role === "ramp") return stats.rampCount <= (structure?.manaFit?.rampRange?.min ?? settings.rampTarget);
  if (role === "removal") return stats.removalCount <= settings.removalTarget;
  if (role === "boardWipe") return stats.boardWipeCount <= settings.wipesTarget;
  const roleStatusItem = structure?.roleBalance?.find((item) => item.key === role);
  return roleStatusItem?.status === "bad" || roleStatusItem?.status === "warn";
}

function roleIsRedundant(role, stats, settings, structure) {
  if (role === "ramp") return stats.rampCount > (structure?.manaFit?.rampRange?.max ?? settings.rampTarget + 2);
  if (role === "removal") return stats.removalCount > settings.removalTarget + 2;
  if (role === "boardWipe") return stats.boardWipeCount > settings.wipesTarget + 1;
  const roleStatusItem = structure?.roleBalance?.find((item) => item.key === role);
  return Boolean(roleStatusItem && roleStatusItem.count > roleStatusItem.goodAt + 2);
}

function cardAddressesWeakCategory({ weakCategories, roles, sharedSignals, isLandCandidate = false }) {
  return weakCategories.some((key) => {
    if (key === "mana") return roles.ramp || roles.manaFixing || isLandCandidate;
    if (key === "flow") return roles.draw || roles.cardSelection || roles.engine || roles.tutor;
    if (key === "interaction") return roles.removal;
    if (key === "boardControl") return roles.boardWipe;
    if (key === "resilience") return roles.protection || roles.recursion;
    if (key === "winPlan") return roles.finisher || roles.payoff;
    if (key === "synergy") return sharedSignals.length > 0;
    if (key === "curve") return true;
    return false;
  });
}

function replacementNeedFromGaps(stats, settings, structure, scorecard = []) {
  const manaFit = structure?.manaFit;
  if (manaFit?.status !== "good" && stats.rampCount < (manaFit?.rampRange?.min ?? settings.rampTarget)) return "cheap ramp";
  if (manaFit?.status !== "good" && stats.landCount < (manaFit?.landRange?.min ?? settings.landsMin)) return "land, MDFC, or lower-curve support";
  if (structure?.cardFlowProfile?.status === "bad") return "repeatable draw or card selection";
  if (stats.removalCount < settings.removalTarget) return "flexible targeted removal";
  if (stats.boardWipeCount < settings.wipesTarget) return "board control";
  if (structure?.winPlan?.status === "bad") return "a redundant finisher or payoff";
  const lowScore = [...(scorecard || [])].filter((item) => !item.ignored).sort((a, b) => a.score - b.score)[0];
  if (lowScore?.key === "synergy") return "commander/core support";
  if (lowScore?.key === "curve") return "lower-curve support";
  return "high-synergy commander/core support";
}

function evaluateCutCandidate({ score, card, roles, roleKeys, sharedSignals, scorecard, stats, settings, structure, isLandCandidate = false }) {
  const cutReason = [];
  const keepRisk = [];
  const cmc = card?.cmc ?? 0;
  const importantRoles = CUT_ROLE_KEYS.filter((role) => roles[role]);
  const weakCategories = (scorecard || []).filter((item) => !item.ignored && item.score < 60).map((item) => item.key);
  let cutPressure = 0;
  let keepPressure = 0;
  let protectsDeficit = false;

  if (isLandCandidate) {
    const extraLands = Math.max(0, stats.landCount - ((structure?.manaFit?.landRange?.max ?? settings.landsMax) + 2));
    cutPressure += 3 + extraLands;
    cutReason.push("Mana support is heavy enough for this curve that a land can be considered as a cut.");
  } else if (score.score <= -3) {
    cutPressure += 5;
    cutReason.push("Very low local strategy score.");
  } else if (score.score <= 0) {
    cutPressure += 4;
    cutReason.push("Few visible signals support the deck's primary plan.");
  } else if (score.score <= 2) {
    cutPressure += 3;
    cutReason.push("Low-impact role-player compared with stronger support cards.");
  } else if (score.score <= 4) {
    cutPressure += 1;
    cutReason.push("Moderate strategy score, so this is cuttable if a slot is needed.");
  }

  if (!isLandCandidate && !sharedSignals.length) {
    cutPressure += 2;
    cutReason.push("No commander/core theme overlap was detected.");
  }
  if (!isLandCandidate && cmc >= 5 && score.score <= 2) {
    cutPressure += 2;
    cutReason.push("Expensive slot without enough visible synergy pressure.");
  }
  if (score.note?.includes("competes with commander turn")) {
    cutPressure += 1;
    cutReason.push("Competes with the commander's mana-value turn.");
  }
  if (!isLandCandidate && !importantRoles.length) {
    cutPressure += 2;
    cutReason.push("Does not fill a protected deckbuilding role like ramp, draw, interaction, engine, payoff, or finisher.");
  }
  if (weakCategories.length && !cardAddressesWeakCategory({ weakCategories, roles, sharedSignals, isLandCandidate })) {
    cutPressure += 1;
    cutReason.push("Does not address the lowest-scoring scorecard categories.");
  }

  for (const role of roleKeys) {
    if (roleIsRedundant(role, stats, settings, structure)) {
      cutPressure += 1;
      cutReason.push(`${ROLE_LABELS[role] || role} is already above the current target.`);
    }
    if (roleIsThin(role, stats, settings, structure)) {
      if (["ramp", "removal", "boardWipe", "protection"].includes(role)) protectsDeficit = true;
      keepPressure += role === "ramp" || role === "removal" || role === "boardWipe" ? 3 : 2;
      keepRisk.push(`Fills a thin ${ROLE_LABELS[role] || role} slot.`);
    }
  }
  if (isLandCandidate && !(stats.landCount > (structure?.manaFit?.landRange?.max ?? settings.landsMax) + 2)) {
    keepPressure += 4;
    keepRisk.push("Lands are not heavy enough for this curve to be a safe cut.");
  }
  if (!isLandCandidate && roles.ramp && structure?.manaFit?.status !== "good" && stats.rampCount < (structure?.manaFit?.rampRange?.min ?? settings.rampTarget)) {
    keepPressure += 3;
    keepRisk.push("Mana fit is light, so cutting ramp is risky.");
  }
  if (!isLandCandidate && (roles.fastMana || roles.comboPiece || isGameChangerName(score.name))) {
    keepPressure += 1;
    keepRisk.push("High-impact power card; cutting changes table power more than synergy shape.");
  }
  if (!cutReason.length) cutReason.push("Cut only if playtesting shows this slot underperforming.");

  return {
    cutPressure,
    keepPressure,
    protectsDeficit,
    rank: cutPressure - keepPressure,
    cutReason: [...new Set(cutReason)].slice(0, 4),
    keepRisk: [...new Set(keepRisk)],
  };
}

function buildCutCandidates({ scores, landEntries, cardMap, identityCards, stats, settings, structure, scorecard, deckSizePlan, coreCards = [] }) {
  const replacementNeed = replacementNeedFromGaps(stats, settings, structure, scorecard);
  const canCutLand = stats.landCount > (structure?.manaFit?.landRange?.max ?? settings.landsMax) + 2;

  const nonLandCandidates = [...scores]
    .filter((score) => !score.protected && score.zone !== "commanders")
    .map((score) => {
      const card = findCard(cardMap, score.name);
      const roles = getRoles(card);
      const roleKeys = getRoleKeys(card);
      const sharedSignals = getSharedSignals(card, identityCards);
      const evaluation = evaluateCutCandidate({ score, card, roles, roleKeys, sharedSignals, scorecard, stats, settings, structure });
      const confidence = evaluation.protectsDeficit
        ? "low"
        : evaluation.rank >= 6 && !evaluation.keepRisk.length
        ? "high"
        : evaluation.rank >= 3
          ? "medium"
          : "low";
      const ownReplacementNeed = roleKeys
        .map((role) => ROLE_REPLACEMENT_NEEDS[role])
        .find(Boolean) || replacementNeed;
      ...
      return {
        ...
        roles: roleKeys,
        confidence,
        reasons: evaluation.cutReason,
        riskFlags: evaluation.keepRisk,
        cutReason: evaluation.cutReason,
        keepRisk: evaluation.keepRisk,
        replacementNeed: ownReplacementNeed,
        protected: false,
        cutPressure: evaluation.cutPressure,
        keepPressure: evaluation.keepPressure,
        rank: evaluation.rank,
      };
    });

  const landCandidates = canCutLand
    ? landEntries
      .filter((entry) => !isCoreName(entry.name, coreCards))
      .map((entry) => {
        const card = findCard(cardMap, entry.name);
        const score = { name: entry.name, score: 0, note: "" };
        const evaluation = evaluateCutCandidate({
          score,
          card,
          roles: {},
          roleKeys: ["land"],
          ...
          settings,
          structure,
          isLandCandidate: true,
        });
        return {
          name: entry.name,
          score: 0,
          roles: ["land"],
          confidence: evaluation.rank >= 6 ? "high" : "medium",
          reasons: evaluation.cutReason,
          riskFlags: evaluation.keepRisk,
          cutReason: evaluation.cutReason,
          keepRisk: evaluation.keepRisk,
  ...
  return [...nonLandCandidates, ...landCandidates]
    .sort((a, b) => b.rank - a.rank || a.score - b.score || a.name.localeCompare(b.name))
    .map((candidate, index) => ({
      ...candidate,
      sizeCutRecommended: index < (deckSizePlan?.cutsNeeded || 0),
    }));
}

function buildUpgrades(stats, cutCandidates, sideboardAnalysis, consideringAnalysis, settings, manaFit) {
  const cuts = [...cutCandidates].filter((candidate) => candidate.confidence !== "low").slice(0, 8);
  const adds = [...sideboardAnalysis, ...consideringAnalysis]
    .filter((item) => item.recommendation === "add")
    .filter((item, index, list) => list.findIndex((candidate) => normalizeName(candidate.name) === normalizeName(item.name)) === index);

  if (adds.length) {
    const deficitRoles = new Set([
      stats.rampCount <= (manaFit?.rampRange?.min ?? settings.rampTarget) ? "ramp" : null,
      stats.removalCount <= settings.removalTarget ? "removal" : null,
      stats.boardWipeCount <= settings.wipesTarget ? "boardWipe" : null,
    ].filter(Boolean));
    const availableCuts = [...cuts];
    return adds.slice(0, 5).map((add) => {
      const addRoles = new Set(add.roles || []);
      let cutIndex = availableCuts.findIndex((candidate) => !(candidate.roles || []).some((role) => deficitRoles.has(role) && !addRoles.has(role)));
      if (cutIndex < 0) cutIndex = 0;
      const cut = availableCuts.splice(cutIndex, 1)[0] || { name: "Lowest-impact slot", score: 0 };
      return {
        cut: cut.name,
        cutScore: cut.score,
        add: add.name,
        addRoles: add.roles || [],
        benefit: add.benefit || add.reason,
        cutReason: cut.cutReason?.[0] || cut.reasons?.[0] || "Lowest-ranked available slot.",
        reason: `${add.benefit || add.reason} Replace ${cut.name} because ${cut.cutReason?.[0] || cut.reasons?.[0] || "it is the lowest-ranked available slot."}`,
        expensive: false,
      };
    });
  }

  const suggestedAdd = manaFit?.status !== "good" && stats.rampCount < (manaFit?.rampRange?.min ?? Math.max(0, settings.rampTarget - 2))
      ? "Additional two-mana ramp"
      : manaFit?.status !== "good" && stats.landCount < (manaFit?.landRange?.min ?? settings.landsMin)
        ? "Land, MDFC, or lower-curve support"
  ...
  return cuts.slice(0, 4).map((cut) => ({
    ...
    cutScore: cut.score,
    add: suggestedAdd,
    benefit: `Improves ${suggestedAdd.toLowerCase()}.`,
    cutReason: cut.cutReason?.[0] || cut.reasons?.[0] || "Lower-impact slot.",
    reason: `Add ${suggestedAdd}. Replace ${cut.name} because ${cut.cutReason?.[0] || cut.reasons?.[0] || "it is the lowest-ranked available slot."}`,
    expensive: false,
  }));
}
...
function buildScorecard({ deck, cardMap, nonLandEntries, stats, structure, bracket, settings, coreCards, identityCards }) {
  ...
  const manaFitScore = manaFit.status === "good"
    ? 90
    : manaFit.status === "warn"
      ? 64
      : 38;
  ...
  return [
    makeScorecardItem(
      ...
      "Mana Fit",
      manaFitScore,
      manaFit.recommendation,
      [
        `${manaFit.currentLands} lands; curve-fit range ${manaFit.landRange.min}-${manaFit.landRange.max}.`,
        `${manaFit.currentRamp} ramp; curve-fit range ${manaFit.rampRange.min}-${manaFit.rampRange.max}.`,
        ...manaFit.reasons.slice(0, 2),
        buildSplashNote(buildColorPips([...deck.commanders, ...nonLandEntries], cardMap)),
      ],
      [manaFit.recommendation],
      [
      ...
      ["landsMin", "landsMax", "rampTarget", "avgManaValueTarget"],
...
function buildCommanderCategoryScores(categories) {
  const scored = Object.entries(categories).map(([category, detail]) => ({
    category,
    rawScore: detail.score,
    squaredScore: detail.score * detail.score,
    evidence: detail.evidence,
  }));
  const totalSquared = scored.reduce((sum, item) => sum + item.squaredScore, 0);
  return scored
    .map((item) => ({
      ...item,
      confidenceScore: totalSquared ? Math.round((item.squaredScore / totalSquared) * 100) : 0,
    }))
    .sort((a, b) => b.rawScore - a.rawScore || a.category.localeCompare(b.category));
}

function commanderCategories() {
  return {
...
function extractCommanderFeatures(entry, cardMap, nonLandEntries, coreCardData) {
  const card = findCard(cardMap, entry.name);
  const text = getCardText(card);
  const roles = getRoles(card);
  const type = getTypeLine(card);
  ...
  return {
    ...
    staxCards,
    protectionCards,
    hasEnablerText: /becomes? (an? )?.*creature|target .* becomes|animate|crew|vehicle|you may cast|you may play|spells? you cast cost .* less|costs? .* less|add .* mana|mana of any color/.test(text),
    hasMultiplierText: /twice that much|twice .* damage|deals? twice|double|doubles?|additional|\+\d+\/\+\d+/.test(text),
    hasValueText: /copy|create .* token|draw .* card|counter on|counters on/.test(text),
    hasCounterweightText: /opponents? can't|players can't|spells .* cost .* more|prevent|protection from|hexproof|ward|indestructible|sacrifice|skip|doesn't untap|enters? .* tapped/.test(text),
    hasRepeatableEngineText: /whenever you|whenever .* you control|whenever .* cast|the first time|at the beginning of/.test(text),
    isHigherCostCreature: type.includes("creature") && (card?.cmc ?? 0) >= 5,
  };
}

function applyCommanderDecisionTree(categories, features) {
  const {
    card,
    text,
    roles,
    ...
    hasMultiplierText,
    hasValueText,
    hasCounterweightText,
    hasRepeatableEngineText,
    isHigherCostCreature,
  } = features;
  ...
  if (coreSupport.length >= 2) {
    pushCommanderSignal(categories.Linchpin, 4, "Selected core cards share this commander's engine signals.", coreSupport);
  ...
  if (hasRepeatableEngineText && !hasMultiplierText) {
    pushCommanderSignal(categories.Linchpin, 1, "Commander has repeatable engine text.");
  }

  if (hasMultiplierText) {
    pushCommanderSignal(categories.Intensifier, 6, "Commander multiplies output, adds extra value, or makes numbers bigger.");
  } else if (roles.draw || roles.tokenMaker || roles.payoff || roles.engine || roles.finisher || hasValueText) {
...
function classifyCommander(entry, cardMap, nonLandEntries, coreCardData) {
  const features = extractCommanderFeatures(entry, cardMap, nonLandEntries, coreCardData);
  const categories = commanderCategories();

  applyCommanderDecisionTree(categories, features);

  const categoryScores = buildCommanderCategoryScores(categories);
  const ranked = categoryScores.map((item) => ({
  ...
  return {
    ...
    outlier,
    score: best.score,
    categoryScores: categoryScores.map(({ category, rawScore, confidenceScore }) => ({ category, rawScore, confidenceScore })),
...
function buildCommanderProfile(deck, cardMap, nonLandEntries, coreCards, coreCardData) {
  const commanders = deck.commanders.map((entry) => classifyCommander(entry, cardMap, nonLandEntries, coreCardData));
...
const NEXT_STEP_PRIORITY = { required: 0, recommended: 1, optional: 2 };

function recommendationPriority(status) {
  return status === "bad" || status === "critical" ? "required" : "recommended";
}

function buildNextSteps({ deckSizePlan, manaFit, interactionProfile, cardFlowProfile, resilienceProfile, winPlan, upgrades, cutCandidates, answerGaps }) {
  const nextByCategory = new Map();
  const add = (step) => {
    if (!nextByCategory.has(step.category)) nextByCategory.set(step.category, step);
  };
  const topCuts = (cutCandidates || []).slice(0, Math.max(3, deckSizePlan?.cutsNeeded || 0));

  if (deckSizePlan?.cutsNeeded > 0) {
    const count = deckSizePlan.cutsNeeded;
    add({
      id: "required-size-cuts",
      category: "cuts",
      priority: "required",
      title: `Review ${count} cut suggestion${count === 1 ? "" : "s"}`,
      summary: `${deckSizePlan.totalCards} cards total; Commander decks target ${deckSizePlan.targetTotal}.`,
      action: `Review the ${count} highest-ranked cut suggestion${count === 1 ? "" : "s"}.`,
      tab: "cuts",
      relatedCards: topCuts.map((candidate) => candidate.name),
    });
  }

  if (manaFit?.status !== "good") {
    const rampLow = manaFit.rampDeficit > 0;
    const landLow = manaFit.landDeficit > 0;
    const action = manaFit.recommendation;
    const title = rampLow && landLow
      ? "Strengthen mana support"
      : rampLow
        ? "Strengthen ramp"
        : landLow
          ? "Strengthen lands"
    ...
    add({
      id: "mana-support",
      category: "mana",
      priority: recommendationPriority(manaFit.status),
      title,
      summary: `${manaFit.currentLands} lands and ${manaFit.currentRamp} ramp; target ${manaFit.landRange.min}–${manaFit.landRange.max} lands and ${manaFit.rampRange.min}–${manaFit.rampRange.max} ramp.`,
      action,
      tab: "mana",
      relatedCards: [],
    });
  ...
  if (interactionProfile?.status === "bad") {
    const gaps = (answerGaps || []).filter((gap) => gap.severity === "gap").slice(0, 2);
    add({
      id: "interaction-coverage",
      category: "interaction",
      priority: "recommended",
      title: "Add flexible interaction",
      summary: gaps.length ? `${gaps.map((gap) => `${gap.label}: ${gap.count}`).join("; ")}.` : "The deck has too few reliable answers.",
      action: "Add answers that cover the listed gaps.",
      tab: "structure",
      relatedCards: interactionProfile.examples || [],
    });
  }
  ...
  if (cardFlowProfile?.status === "bad") {
    add({
      ...
      category: "flow",
      priority: "recommended",
      title: "Improve card flow",
      summary: `${cardFlowProfile.draw} draw, ${cardFlowProfile.tutors} tutors, and ${cardFlowProfile.selection} selection effects detected.`,
      action: "Add repeatable draw or card selection.",
      tab: "structure",
      relatedCards: [],
    });
  }
  ...
  if (resilienceProfile?.status === "bad") {
    add({
      ...
      summary: `${resilienceProfile.protection} protection and ${resilienceProfile.recursion} recursion effects support rebuilding.`,
      action: "Add protection or recursion for key engines.",
      tab: "structure",
      relatedCards: [],
    });
  }
  ...
  if (winPlan?.status === "bad") {
    add({
      ...
      category: "win-plan",
      priority: "recommended",
      title: "Clarify the finish",
      summary: "Few finishers or payoff cards are visible in the current list.",
      action: "Add a finisher tied to the deck's primary engine.",
      tab: "overview",
      relatedCards: [],
    });
  }
  ...
  if ((upgrades || []).length) {
    ...
    add({
      id: "test-upgrade-swap",
      category: "upgrades",
      priority: "optional",
      title: "Test one upgrade swap",
      summary: `${firstUpgrade.add} is paired with ${firstUpgrade.cut}.`,
      action: `Test ${firstUpgrade.add} over ${firstUpgrade.cut}.`,
      tab: "upgrades",
      relatedCards: [firstUpgrade.add, firstUpgrade.cut].filter(Boolean),
    });
  }

  return [...nextByCategory.values()]
    .sort((left, right) => NEXT_STEP_PRIORITY[left.priority] - NEXT_STEP_PRIORITY[right.priority] || left.category.localeCompare(right.category));
}

function buildRoadmap({ cutCandidates, upgrades, structure, deckSizePlan, coreCards, nextSteps }) {

  const cutPriorities = (cutCandidates || []).slice(0, Math.max(3, deckSizePlan?.cutsNeeded || 0)).map((candidate) => ({
    name: candidate.name,
    confidence: candidate.confidence,
    reason: candidate.cutReason?.[0] || candidate.reasons?.[0] || "Lower-ranked fit for the current strategy.",
    replacementNeed: candidate.replacementNeed,
    required: Boolean(candidate.sizeCutRecommended),
  }));

  const upgradePairs = (upgrades || []).slice(0, 3).map((upgrade) => ({
    cut: upgrade.cut,
    add: upgrade.add,
  ...
  const protect = [
    ...(coreCards || []),
    ...(structure?.winPlan?.engines || []).slice(0, 3),
    ...(structure?.winPlan?.payoffs || []).slice(0, 3),
  ].filter(Boolean);

  return {
    status: (nextSteps || []).some((step) => step.priority === "required") ? "needs-work" : (nextSteps || []).length ? "tune" : "stable",
    headline: nextSteps?.[0]?.action || "Tune from matchup notes and actual games.",
    steps: (nextSteps || []).map((step) => ({ label: step.title, action: step.action, reason: step.summary, tab: step.tab })),
    cutPriorities,
    upgradePairs,
    protect: [...new Set(protect)].slice(0, 8),
  };
}

function buildActionPlan({ cutCandidates, upgrades, roadmap, deckSizePlan, nextSteps }) {
  const tasks = (nextSteps || []).map((step) => ({
    id: step.id,
    type: step.category === "cuts" ? "cut" : step.category === "upgrades" ? "add" : "fix",
    priority: step.priority,
    label: step.title,
    detail: step.summary,
    action: step.action,
    tab: step.tab,
    relatedCards: step.relatedCards || [],
  }));
  const requiredCuts = (cutCandidates || []).filter((candidate) => candidate.sizeCutRecommended);
  const topCuts = requiredCuts.length
    ? requiredCuts
    : (cutCandidates || []).filter((candidate) => candidate.confidence !== "low").slice(0, 3);
  const topAdds = (upgrades || []).slice(0, 3);

  const requiredCount = tasks.filter((task) => task.priority === "required").length;
  const recommendedCount = tasks.filter((task) => task.priority === "recommended").length;
  const headline = requiredCount
    ? `${requiredCount} required fix${requiredCount === 1 ? "" : "es"} before this list is table-ready.`
    : recommendedCount
      ? `${recommendedCount} recommended tuning task${recommendedCount === 1 ? "" : "s"} to improve the draft.`
      : "No required fixes are active; use playtest notes for the next tuning pass.";

  return {
    status: requiredCount ? "required" : recommendedCount ? "tune" : "stable",
    headline,
    requiredCount,
    recommendedCount,
    tasks: tasks.slice(0, 6),
    nextCuts: topCuts.slice(0, Math.max(3, deckSizePlan?.cutsNeeded || 0)).map((candidate) => ({
      name: candidate.name,
      confidence: candidate.confidence,
      rank: candidate.rank,
      required: Boolean(candidate.sizeCutRecommended),
      reason: candidate.cutReason?.[0] || candidate.reasons?.[0] || "Lower strategic fit than the cards around it.",
      replacementNeed: candidate.replacementNeed,
    })),
    nextAdds: topAdds.map((upgrade) => ({
      add: upgrade.add,
      cut: upgrade.cut,
      reason: upgrade.reason,
    })),
    protect: roadmap?.protect || [],
  };
}

export function buildLocalAnalysis(deck, rawCardMap, options = {}) {
  const cardMap = applyRoleOverridesToCardMap(rawCardMap, options.roleOverrides || {});
  ...
  const deckSizePlan = buildDeckSizePlan(deck);
  const manaFit = buildManaFit(nonLandEntries, cardMap, commanderCards, stats, settings);
  const colorPips = buildColorPips([...deck.commanders, ...nonLandEntries], cardMap);
  const genericMana = buildGenericManaCost([...deck.commanders, ...nonLandEntries], cardMap);
  const synergyClusters = buildSynergyClusters(nonLandEntries, cardMap, identityCards, coreCards);
  const scoreOptions = { coreCards, commanderNames: deck.commanders.map((entry) => entry.name), settings, manaFit };
  ...
  const roleBalance = buildRoleBalance(nonLandEntries, cardMap, settings, manaFit);
  ...
  const answerGaps = buildAnswerGaps(stats, interactionProfile, resilienceProfile, commanderCards);
  ...
  const cutCandidates = buildCutCandidates({ scores, landEntries, cardMap, identityCards, stats, settings, structure, scorecard, deckSizePlan, coreCards });
  const highlights = buildHighlights(scorecard);
  const priorityFindings = buildPriorityFindings(stats, interactionProfile, resilienceProfile, cardFlowProfile, winPlan, bracket, scorecard, answerGaps, manaFit, deckSizePlan);
  const upgrades = buildUpgrades(stats, cutCandidates, sideboardAnalysis, consideringAnalysis, settings, manaFit);
  const nextSteps = buildNextSteps({ deckSizePlan, manaFit, interactionProfile, cardFlowProfile, resilienceProfile, winPlan, upgrades, cutCandidates, answerGaps });
  const roadmap = buildRoadmap({ cutCandidates, upgrades, structure, deckSizePlan, coreCards, nextSteps });
  const actionPlan = buildActionPlan({ cutCandidates, upgrades, roadmap, deckSizePlan, nextSteps });
  ...
  return {
    strategy: `Set up around ${clusterText}, establish the deck's engines, then convert that board into a finish.`,
    stats,
    deckSizePlan,
    manaFit,
    colorPips,
    ...
    consistencyFlags: buildConsistencyFlags(stats, deck, settings, manaFit),
    synergyClusters,
    weaknesses: buildWeaknesses(stats, settings, manaFit),
    scores,
    sideboardAnalysis,
    consideringAnalysis,
    cutCandidates,
...
export function mergeAnalysis(remoteAnalysis, localAnalysis) {
  ...
  return {
    ...
    deckSizePlan: localAnalysis.deckSizePlan,
    manaFit: localAnalysis.manaFit,
    colorPips: { ...localAnalysis.colorPips, ...(remoteAnalysis.colorPips || {}) },
    genericMana: localAnalysis.genericMana,
    ...
    scores: Array.isArray(remoteAnalysis.scores)
      ? remoteAnalysis.scores.map((score) => {
        ...
        return { ...localScore, ...score, roles: localScore?.roles || score.roles || [], roleEvidence: localScore?.roleEvidence || score.roleEvidence || [] };
      })
    ...
    cutCandidates: localAnalysis.cutCandidates,
    upgrades: localAnalysis.upgrades,
    nextSteps: localAnalysis.nextSteps,
    roadmap: localAnalysis.roadmap,
