...
function classifyCommander(entry, cardMap, nonLandEntries, coreCards, coreCardData) {
  ...
  if (/twice that much|twice .* damage|deals? twice|double|doubles?|additional|\+\d+\/\+\d+/.test(text)) {
    pushCommanderSignal(categories.Intensifier, 6, "Commander multiplies output, adds extra value, or makes numbers bigger.");
  } else if (roles.draw || roles.tokenMaker || roles.payoff || roles.engine || roles.finisher || /copy|create .* token|draw .* card|counter on|counters on/.test(text)) {
