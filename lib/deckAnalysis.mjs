...
function buildPriorityFindings(stats, interactionProfile, resilienceProfile, cardFlowProfile, winPlan, bracket, scorecard = [], answerGaps = [], manaFit) {
  const findings = [];
...
function describeCutReasons({ score, card, roles, sharedSignals, scorecard, settings }) {
  const reasons = [];
  const cmc = card?.cmc ?? 0;
  const importantRoles = ["ramp", "draw", "removal", "boardWipe", "protection", "recursion", "engine", "payoff", "finisher"].filter((role) => roles[role]);

  if (score.score <= -3) reasons.push("Very low local strategy score.");
  else if (score.score <= 0) reasons.push("Few visible signals support the deck's primary plan.");
  if (!sharedSignals.length) reasons.push("No commander/core theme overlap was detected.");
  if (cmc >= 5 && score.score <= 2) reasons.push("Expensive slot without enough visible synergy pressure.");
  if (score.note?.includes("competes with commander turn")) reasons.push("Competes with the commander's mana-value turn.");
  if (!importantRoles.length) reasons.push("Does not fill a protected deckbuilding role like ramp, draw, interaction, engine, payoff, or finisher.");

  const weakCategories = (scorecard || []).filter((item) => !item.ignored && item.score < 60).map((item) => item.key);
  const helpsKnownGap =
    (weakCategories.includes("mana") && roles.ramp) ||
    (weakCategories.includes("cardFlow") && (roles.draw || roles.cardSelection || roles.engine || roles.tutor)) ||
    (weakCategories.includes("interaction") && roles.removal) ||
    (weakCategories.includes("boardControl") && roles.boardWipe) ||
    (weakCategories.includes("resilience") && (roles.protection || roles.recursion)) ||
    (weakCategories.includes("winPlan") && (roles.finisher || roles.payoff));
  if (weakCategories.length && !helpsKnownGap) reasons.push("Does not address the lowest-scoring scorecard categories.");
  if (!reasons.length && score.score <= 3) reasons.push("Lower-impact role-player compared with the deck's stronger support cards.");
  if (!reasons.length && settings) reasons.push("Cut only if playtesting shows this slot underperforming.");

  return reasons.slice(0, 4);
}

function buildCutCandidates({ scores, cardMap, identityCards, stats, settings, structure, scorecard }) {
  ...
  return [...scores]
    ...
    .map((score) => {
      ...
      if (score.score <= -3) pressure += 4;
      else if (score.score <= 0) pressure += 3;
      else if (score.score <= 2) pressure += 2;
      else if (score.score <= 4) pressure += 1;
      if (!sharedSignals.length) pressure += 2;
      if ((card?.cmc ?? 0) >= 5 && score.score <= 2) pressure += 2;
      if (score.note?.includes("competes with commander turn")) pressure += 1;
      if (!roleKeys.some((role) => ["ramp", "draw", "removal", "boardWipe", "protection", "recursion", "engine", "payoff", "finisher"].includes(role))) pressure += 2;

      for (const role of roleKeys) {
        if (roleIsThin(role, stats, settings, structure)) {
          riskFlags.push(`Fills a thin ${ROLE_LABELS[role] || role} slot.`);
          pressure -= 2;
        }
      }
      if (roles.fastMana || roles.comboPiece || isGameChangerName(score.name)) {
        riskFlags.push("High-impact power card; cutting changes table power more than synergy shape.");
        pressure -= 1;
      }

      const confidence = pressure >= 6 && !riskFlags.length
        ? "high"
        : pressure >= 4
          ? "medium"
          : "low";
      ...
      return {
        ...
        roles: roleKeys,
        confidence,
        reasons: describeCutReasons({ score, card, roles, sharedSignals, scorecard, settings }),
    ...
    .sort((a, b) => b.cutPressure - a.cutPressure || a.score - b.score || a.name.localeCompare(b.name))
    .map(({ cutPressure, ...candidate }) => candidate);
}
...
export function buildLocalAnalysis(deck, cardMap, options = {}) {
  ...
  const cutCandidates = buildCutCandidates({ scores, cardMap, identityCards, stats, settings, structure, scorecard });
  const highlights = buildHighlights(scorecard);
  const priorityFindings = buildPriorityFindings(stats, interactionProfile, resilienceProfile, cardFlowProfile, winPlan, bracket, scorecard, answerGaps, manaFit);
  ...
  return {
    ...
    consideringAnalysis,
    cutCandidates,
    upgrades: buildUpgrades(stats, cutCandidates, sideboardAnalysis, consideringAnalysis, settings, manaFit),
    bracket,
    commanderProfile,
    structure,
    cardGroups,
