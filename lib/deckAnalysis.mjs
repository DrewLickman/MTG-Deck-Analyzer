...
function roleIsThin(role, stats, settings, structure) {
  if (role === "ramp") return stats.rampCount < settings.rampTarget;
...
function replacementNeedFromGaps(stats, settings, structure, scorecard = []) {
  if (stats.rampCount < settings.rampTarget) return "cheap ramp";
...
function describeCutReasons({ score, card, roles, sharedSignals, scorecard, settings }) {
  ...
  const helpsKnownGap =
    (weakCategories.includes("ramp") && roles.ramp) ||
...
function buildUpgrades(stats, cutCandidates, sideboardAnalysis, consideringAnalysis, settings) {
...
export function buildLocalAnalysis(deck, cardMap, options = {}) {
  ...
  return {
    ...
    consideringAnalysis,
    cutCandidates,
    upgrades: buildUpgrades(stats, cutCandidates, sideboardAnalysis, consideringAnalysis, settings),
    bracket,
    commanderProfile,
    structure,
    cardGroups,
