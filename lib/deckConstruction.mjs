...
export function returnVersusComparison(session, cards = []) {
  if (cards.length !== 2 || cards.some((card) => !hasComparisonCard(session, card))) return session;
  return {
    ...session,
    notice: `Neither selected; ${cards.map((card) => `${card.name} returned to ${comparisonSourceLabel(card.source)}`).join(" and ")}.`,
  };
}
