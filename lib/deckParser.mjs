...
export function validateCommandZone(deck, cardMap, findCard, getCardText) {
  ...
  const validPartnerNames = allEntries
    .filter((entry) => /partner|friends forever|doctor's companion|choose a background/.test(getCardText(findCard(cardMap, entry.name))))
    .map((entry) => entry.name);
  const validCompanionNames = allEntries
    .filter((entry) => /\bcompanion\b/.test(getCardText(findCard(cardMap, entry.name))))
    .map((entry) => entry.name);
  ...
  return {
    ...
    validPartnerNames,
    validCompanionNames,
    hasValidPartner: validPartnerNames.length > 0,
    hasValidCompanion: validCompanionNames.length > 0,
  };
}
