...
export function parseDecklist(raw, options = {}) {
  ...
  if (manualCommanders.length) {
    ...
    finalMainBlock &&
    mainBlocks.length > 1 &&
    finalMainBlock.entries.length >= 1 &&
    finalMainBlock.entries.length <= 2 &&
    finalMainBlock.entries.every((entry) => entry.qty === 1)
  ) {
  ...
  deck.main = removeNames(deck.main, [...deck.commanders, ...deck.companions].map((entry) => entry.name));
