...
function evaluateHand(hand, cardMap, coreCards = []) {
  ...
  score += Math.min(15, summary.earlyCards.length * 6 * densityScale);
  if (summary.earlyCards.length === 0) score -= 12;
  score += Math.min(10, summary.rampCards.length * (summary.coloredSourceCount <= 2 ? 5 : 3) * densityScale);
  score += Math.min(8, summary.flowCards.length * 4 * densityScale);
  score += Math.min(8, summary.engineCards.length * 4 * densityScale);
  score += Math.min(4, summary.interactionCards.length * 2 * densityScale);
  score += Math.min(6, summary.earlyCastable * 2 * densityScale);
  ...
  if (summary.landCount > Math.ceil(handSize * 4 / 7)) concerns.push(`${summary.landCount} lands leaves too little action.`);
...
function glueNeedDefinitions(summary) {
  ...
  return [
    ...
    },
    {
      ...
      label: "Cheap Action",
      priority: 95,
      active: summary.landCount > actionLandCeiling,
...
function reconcileVerdict(baseline, glueNeeds) {
  ...
  if (baseline.verdict.label === "Strong keep" && majorNeeds.length >= 2) {
    return { label: "Keepable", status: "good" };
  }
...
export function analyzeOpeningHand({ deck, hand, cardMap = {}, analysis = {}, coreCards = [] }) {
  ...
  return {
    ...
    glueSummary: glueNeeds.length
      ? `${glueNeeds.map((need) => need.label).join(", ")} are the most useful missing categories for this ${baseline.summary.cards.length}-card hand.`
